...
class Model_attendance extends CI_Model
{
    ...
    public function std_attendance_list($slot_time, $slot_day, $slot_date)
    {
        ...
        $sql = "SELECT * FROM (
				...
				LEFT JOIN student_attendance a ON a.student_id = s.sid AND a.attend_date = '" . $slot_date . "' AND a.void='N'
				...
				LEFT JOIN class_extension_log cel ON cel.student_id = s.sid AND cel.taken_status = 'N'
				...
				LEFT JOIN ( SELECT student_id, COUNT(id) AS lesson_overdue FROM student_attendance
							WHERE bill_id IS NULL AND void='N' GROUP BY student_id ) av ON av.student_id = s.sid
				WHERE attendance_status='Y' AND a.attend_time = '" . $slot_time . "' AND a.attend_date = '" . $slot_date . "'AND replacement = 'Y' AND a.void='N'
    ...
    public function get_next_slot_time($slot_day, $curr_time, $schedule_id)
    {
        if (is_null($schedule_id)) {
            $sql = "SELECT
				...
				WHERE 
					slot_day = ? AND slot_time >= ? AND course_status='A'
				GROUP BY slot_time 
    ...
    public function class_cancellation(){
        ...
        if ($cancellation_type == 1) {
            $sql = "INSERT INTO class_extension_log (student_id, cancellation_id, created_time, modify_time)
                    SELECT
                        student_id, ".$last_id.",'".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."'
                    FROM
                        student_attendance a
                    WHERE a.attend_time = '" . $cancellation_time . "' AND a.attend_date= '" . $cancellation_date . "' AND attendance_status='Y'";

            $query = $this->db->query($sql);

        } else if ($cancellation_type == 2) {
            $sql = "UPDATE student_attendance SET attendance_status = 'N', timestamp = " . time() . ", pic = '" . $pic . "' WHERE attend_date='" . $cancellation_date . "' AND attend_time='" . $cancellation_time . "' AND bill_id IS NOT NULL";
            $this->db->query($sql);

            $sql = "UPDATE student_attendance SET void = 'Y', timestamp = " . time() . ", pic = '" . $pic . "' WHERE attend_date='" . $cancellation_date . "' AND attend_time='" . $cancellation_time . "' AND bill_id IS NULL";
            $this->db->query($sql);
        }
    ...
    public function class_extension_clear(){
        ...
        $sql = "UPDATE class_extension_log set taken_status = 'Y', modify_time = '".date('Y-m-d H:i:s')."' WHERE ce_id = ".$sid;
        $this->db->query($sql);
