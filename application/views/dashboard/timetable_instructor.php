		...
		$('#calendar').fullCalendar({
			...
			<?php 
				...
				foreach($timetable as $row){
					...
					$tempStr .= "{title:'".$row['instructors_name']."', start:'".  date('Y-m-d', strtotime("Last Sunday +". $row['slot_day']." Days")) . "T".$row['slot_time']."',";
					$tempStr .= "end:'".date('Y-m-d', strtotime("Last Sunday +". $row['slot_day']." Days"))."T".$row['end_slot_time']."'}";
									...
									<div class="form-group">
										<select id="venue_id" name="venue_id" class="form-control">
											...
											<option value="<?php echo $venue['venue_id']?>" <?php
												if ($venue['venue_id'] == $venue_id) {echo "selected='selected'";} ?>><?php echo $venue['venue_name']; ?>
											</option>
											<?php } ?>
										</select>
									</div>
